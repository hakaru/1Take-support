...
録音分析は録音停止後に自動実行されます。結果は以下に役立ちます：

1. **クリッピング問題の特定** - トゥルーピークが-1 dBTPを超えた場合
2. **音量のチェック** - インテグレーテッドLUFSをストリーミング基準と比較
3. **ダイナミクスの評価** - クレストファクターでコンプレッション量を確認
4. **テクニックの改善** - 一貫した結果は良いマイクテクニックを示す
...
ドラッグ可能なコントロールポイント付きの周波数レスポンスカーブでEQ設定を視覚的に確認できます。
...
1Take v1.3.0では自動スレッショルド設定を導入：

1. 自動録音設定で**Auto**ボタンをタップ
2. 1Takeが周囲のノイズフロアを測定
3. 最適なマージンでスレッショルドを自動計算
4. ヒステリシス制御で誤トリガーを防止

演奏前の静かな瞬間にAutoをタップするだけで設定完了！
...
1Takeは重要な瞬間を特定するための自動マーカーをサポート：

### ユーザーマーカー
録音中にマーカーボタンをタップして手動で追加。重要な瞬間をマークするのに使用。

### Clipマーカー（CLIP）
音声が0 dBFSを超えた時に自動追加。歪みの可能性がある箇所を特定するのに役立ちます。

### Overマーカー（OVER）
トゥルーピークが-1 dBTPを超えた時に自動追加。再生時に問題を起こす可能性のあるインターサンプルピークを示します。
...
自動録音トリガーを設定。インテリジェント・キャリブレーションには**Auto**ボタンを使用。

### 画面をオンのまま維持（v1.3.0新機能）

長時間の録音セッション中にデバイスがスリープしないようにするオプション。長時間の練習やライブ録音に便利です。
